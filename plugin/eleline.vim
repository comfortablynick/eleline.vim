...
function! ElelineWarning() abort
    if exists('g:did_coc_loaded')
        ...
        if empty(l:coc_info)
            ...
            let l:count = l:coc_info['warning']
        endif
    elseif exists('g:loaded_ale')
        let l:count = ale#statusline#Count(bufnr(''))['1']
    else
        let l:count = 0
    endif
...
function! ElelineError() abort
    if exists('g:did_coc_loaded')
        ...
        if empty(l:coc_info)
            ...
            let l:count = l:coc_info['error']
        endif
    elseif exists('g:loaded_ale')
        let l:count = ale#statusline#Count(bufnr(''))['0']
    else
        let l:count = 0
    endif
    return l:count == 0 ? '' : '•'.l:count.' '
